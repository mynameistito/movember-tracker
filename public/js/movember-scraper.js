...
async function detectSubdomainForMember(memberId, forceRefresh = false) {
	...
	if (!forceRefresh) {
		...
		if (cached) {
			console.log(
				`[SUBDOMAIN] Found cached subdomain for memberId ${memberId}: ${cached}`,
			);
			return cached;
		}
	} else {
		console.log(
			`[SUBDOMAIN] Force refresh requested, skipping cache for memberId ${memberId}`,
		);
	}
	...
	if (MEMBER_SUBDOMAIN_MAP[memberId]) {
		...
		console.log(
			`[SUBDOMAIN] Using manual override for memberId ${memberId}: ${subdomain}`,
		);
	...
	try {
		...
		for (const subdomain of commonSubdomains) {
			const testSubdomainUrl =
				MOVEMBER_BASE_URL_TEMPLATE.replace("{subdomain}", subdomain) +
				`?memberId=${memberId}`;
			try {
				...
				if (testHtml && testHtml.length > 1000) {
					...
					if (actualSubdomain && actualSubdomain !== subdomain) {
						...
						console.log(
							`[SUBDOMAIN] URL redirected from ${subdomain} to ${actualSubdomain} for memberId ${memberId}`,
						);
					...
					if (
						...
						console.log(
							`[SUBDOMAIN] Found matching subdomain for memberId ${memberId}: ${confirmedSubdomain} (verified by currency)`,
						);
						setCachedSubdomain(
							memberId,
							confirmedSubdomain,
						...
						console.log(
							`[SUBDOMAIN] HTML currency indicates ${detectedSubdomain} (tested ${subdomain}, final URL: ${actualSubdomain || subdomain}), using detected subdomain`,
						);
						setCachedSubdomain(
							memberId,
							detectedSubdomain,
						...
						if (!fallbackSubdomain) {
							...
							console.log(
								`[SUBDOMAIN] Found valid HTML for subdomain ${fallbackSubdomainToUse} (currency check inconclusive, storing as fallback)`,
							);
						}
					}
				}
				...
				console.warn("[SUBDOMAIN] Error trying subdomain:", e);
			}
		}
		...
		if (fallbackSubdomain) {
			console.log(
				`[SUBDOMAIN] Using fallback subdomain for memberId ${memberId}: ${fallbackSubdomain} (no currency match found)`,
			);
		...
		const testUrl =
			MOVEMBER_BASE_URL_TEMPLATE.replace("{subdomain}", DEFAULT_SUBDOMAIN) +
			`?memberId=${memberId}`;
		try {
			...
			if (html && html.length > 1000) {
				...
				console.log(
					`[SUBDOMAIN] Using default subdomain for memberId ${memberId}: ${actualSubdomain}`,
				);
				setCachedSubdomain(memberId, actualSubdomain, SUBDOMAIN_CACHE_TTL);
				return actualSubdomain;
			}
			...
			console.warn("[SUBDOMAIN] Error trying default subdomain:", e);
		}

		// Fallback to default
		console.warn(
			`[SUBDOMAIN] Could not find working subdomain for memberId ${memberId}, using default: ${DEFAULT_SUBDOMAIN}`,
		);
		setCachedSubdomain(memberId, DEFAULT_SUBDOMAIN, SUBDOMAIN_CACHE_TTL);
		return DEFAULT_SUBDOMAIN;
	} catch (error) {
		console.warn(
			`[SUBDOMAIN] Failed to detect subdomain for memberId ${memberId}, using default:`,
			error,
		);
		setCachedSubdomain(memberId, DEFAULT_SUBDOMAIN, SUBDOMAIN_CACHE_TTL);
		return DEFAULT_SUBDOMAIN;
...
export async function scrapeMovemberPage(
	...
	console.log(
		`[SCRAPE] Starting scrape of Movember page: ${movemberUrl} (subdomain: ${subdomain})`,
	);
	...
	try {
		...
		try {
			...
			if (clearSubdomainOn404 && error.message.includes("404")) {
				console.warn(
					`[SCRAPE] Got 404 for ${movemberUrl}, clearing cached subdomain and re-detecting...`,
				);
				...
				if (newSubdomain !== subdomain) {
					console.log(
						`[SCRAPE] Re-detected subdomain: ${newSubdomain} (was ${subdomain}), retrying with new subdomain...`,
					);
					// Retry with new subdomain
					const newUrl =
						MOVEMBER_BASE_URL_TEMPLATE.replace("{subdomain}", newSubdomain) +
						`?memberId=${memberId}`;
		...
		console.log(
			`[SCRAPE] HTML fetched successfully in ${formatDuration(fetchDuration)} (${html.length} characters)`,
		);
		...
		if (actualSubdomain && actualSubdomain !== subdomain) {
			console.log(
				`[SCRAPE] URL redirected from ${subdomain} to ${actualSubdomain}, updating subdomain...`,
			);
		...
		if (htmlDetectedSubdomain && htmlDetectedSubdomain !== subdomain) {
			console.warn(
				`[SCRAPE] HTML currency indicates subdomain ${htmlDetectedSubdomain} but URL subdomain is ${subdomain}. Trusting URL subdomain (primary source).`,
			);
			// Don't override - trust the URL subdomain we're using
		} else if (htmlDetectedSubdomain === subdomain) {
			console.log(
				`[SCRAPE] HTML currency verification confirms subdomain ${subdomain}`,
			);
		}
		...
		for (let i = 0; i < raisedPatterns.length; i++) {
			...
			if (match) {
				...
				console.log(
					`[SCRAPE] Pattern ${i + 1} matched, all groups:`,
					match.slice(1),
					`using: "${captured}"`,
				);
				...
				if (isValidNumber(captured)) {
					raised = captured;
					console.log(
						`[SCRAPE] Found valid raised amount using pattern ${i + 1}: ${raised}`,
					);
					break;
				} else {
					console.warn(
						`[SCRAPE] Pattern ${i + 1} matched but invalid number: "${captured}", trying next pattern...`,
					);
				}
			}
		}
		...
		for (let i = 0; i < targetPatterns.length; i++) {
			...
			if (match) {
				...
				console.log(
					`[SCRAPE] Target pattern ${i + 1} matched, all groups:`,
					match.slice(1),
					`using: "${captured}"`,
				);
				...
				if (isValidNumber(captured)) {
					target = captured;
					console.log(
						`[SCRAPE] Found valid target amount using pattern ${i + 1}: ${target}`,
					);
					break;
				} else {
					console.warn(
						`[SCRAPE] Target pattern ${i + 1} matched but invalid number: "${captured}", trying next pattern...`,
					);
				}
			}
		}
		...
		if (!raised || !target) {
			...
			const scriptTagMatches = html.match(
						...
						for (let i = 0; i < raisedJsonPatterns.length; i++) {
							...
							if (match) {
								...
								if (isValidNumber(captured)) {
									raised = captured;
									console.log(
										`[SCRAPE] Found valid raised amount in JSON using pattern ${i + 1}: ${raised}`,
									);
									break;
								} else {
									console.warn(
										`[SCRAPE] JSON raised pattern ${i + 1} matched but invalid number: "${captured}"`,
									);
								}
							}
						}
						...
						for (let i = 0; i < targetJsonPatterns.length; i++) {
							...
							if (match) {
								...
								if (isValidNumber(captured)) {
									target = captured;
									console.log(
										`[SCRAPE] Found valid target amount in JSON using pattern ${i + 1}: ${target}`,
									);
									break;
								} else {
									console.warn(
										`[SCRAPE] JSON target pattern ${i + 1} matched but invalid number: "${captured}"`,
									);
								}
							}
						}
		...
		if (!raised) {
			console.log(
				`[SCRAPE] Trying generic dollar amount patterns as last resort...`,
			);
			...
			for (let i = 0; i < genericPatterns.length; i++) {
				...
				if (match) {
					...
					if (isValidNumber(captured)) {
						raised = captured;
						console.log(
							`[SCRAPE] Found valid raised amount using generic pattern ${i + 1}: ${raised}`,
						);
						break;
					} else {
						console.warn(
							`[SCRAPE] Generic pattern ${i + 1} matched but invalid number: "${captured}"`,
						);
					}
				}
			}
		...
		if (!target) {
			console.log(
				`[SCRAPE] Trying generic target amount patterns as last resort...`,
			);
			...
			for (let i = 0; i < genericTargetPatterns.length; i++) {
				...
				if (match) {
					...
					if (isValidNumber(captured)) {
						target = captured;
						console.log(
							`[SCRAPE] Found valid target amount using generic pattern ${i + 1}: ${target}`,
						);
						break;
					} else {
						console.warn(
							`[SCRAPE] Generic target pattern ${i + 1} matched but invalid number: "${captured}"`,
						);
					}
				}
			}
		...
		console.log(
			`[SCRAPE] Data extraction completed in ${formatDuration(extractDuration)}`,
		);
		console.log(
			`[SCRAPE] Raw extracted data for memberId ${memberId} (subdomain: ${subdomain}):`,
			{
		...
		if (!raised || !target) {
			...
			console.log(
				`[SCRAPE] Found ${allDollarMatches.length} dollar amounts in HTML`,
			);
			...
			if (allDollarMatches.length > 0) {
				...
				if (!raised && scoredAmounts.length > 0) {
					...
					if (raisedCandidates.length > 0) {
						...
						console.log(
							`[SCRAPE] Found raised amount via context search: ${raised} (raisedScore: ${raisedCandidates[0].raisedScore})`,
						);
					}
				}
				...
				if (!target && scoredAmounts.length > 0) {
					...
					if (targetCandidates.length > 0) {
						...
						console.log(
							`[SCRAPE] Found target amount via context search: ${target} (targetScore: ${targetCandidates[0].targetScore})`,
						);
					}
				}
			}
		...
		if (!raised || !isValidNumber(raised)) {
			...
			console.warn(
				`[SCRAPE] Found ${allDollarAmounts ? allDollarAmounts.length : 0} dollar amounts in HTML:`,
			...
			console.warn(
				`[SCRAPE] Found ${potentialAmounts ? potentialAmounts.length : 0} potential amount numbers in HTML (showing first 20):`,
		...
		if (!raisedValue || raisedValue === "0" || raisedValue === "") {
			console.warn(
				`[SCRAPE] Parsed raised value is invalid: "${raisedValue}" from input: "${raised}"`,
			);
		}
		...
		if (target && isValidNumber(target)) {
			...
			console.warn(
				`[SCRAPE] Target value "${target}" failed validation, skipping target`,
			);
		}

		const totalDuration = Date.now() - startTime;
		console.log(
			`[SCRAPE] Scraping completed successfully in ${formatDuration(totalDuration)}:`,
			{
		...
		console.error(
			`[SCRAPE] Scraping failed after ${formatDuration(totalDuration)}:`,
			errorMessage,
			error,
		);
		throw error;
...
export async function scrapeWithRetry(memberId) {
	...
	console.log(
		`[RETRY] Starting retry logic (max ${MAX_RETRIES} attempts) for memberId: ${memberId}`,
	);
	...
	for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
		try {
			...
			console.log(
				`[RETRY] Success on attempt ${attempt + 1} after ${totalDuration}ms`,
			);
			return result;
		} catch (error) {
			...
			if (errorMessage.includes("404")) {
				console.log(
					`[RETRY] 404 detected, clearing subdomain cache for memberId: ${memberId}`,
				);
				clearSubdomainCache(memberId);
			}
			...
			if (attempt < MAX_RETRIES - 1) {
				...
				console.log(
					`[RETRY] Waiting ${delay}ms before retry ${attempt + 2}...`,
				);
				await sleep(delay);
			} else {
				const totalDuration = Date.now() - retryStartTime;
				console.error(
					`[RETRY] All ${MAX_RETRIES} attempts failed after ${totalDuration}ms`,
				);
			}
		}
	}
...
export async function getData(memberId, grabLive = false) {
	...
	if (grabLive) {
		...
		console.log(
			`[LIVE] grab-live parameter detected - forcing fresh scrape for memberId: ${memberId}`,
		);
		...
		console.log(
			`[CACHE] Storing live data in cache with TTL: ${CACHE_TTL}ms for memberId: ${memberId}`,
		);
		...
		if (data) {
			...
			console.log(
				`[CACHE] Cache HIT - data age: ${Math.round(cacheAge / 1000)}s for memberId: ${memberId}`,
				{
					amount: data.amount,
					target: data.target,
			...
			console.log(
				`[CACHE] Cache MISS - need to scrape for memberId: ${memberId}`,
			);
		}
		...
		if (!data) {
			...
			// Store in cache with 5-minute TTL
			console.log(
				`[CACHE] Storing data in cache with TTL: ${CACHE_TTL}ms for memberId: ${memberId}`,
			);
			setCachedData(memberId, data, CACHE_TTL);
